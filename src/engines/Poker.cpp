...
HandValue evaluate5Cards(QVector<Card> c) {
    ...
    const bool isFlush = (c[0].suit == c[1].suit && c[1].suit == c[2].suit &&
...
HandValue evaluate7Cards(const QVector<Card>& cards) {
    ...
    // Оптимизированный алгоритм: перебор ровно 21 сочетания 5 из 7
    if (cards.size() == 7) {
        for (int i = 0; i < 7; ++i) {
            for (int j = i + 1; j < 7; ++j) {
                QVector<Card> combo;
                ...
                const HandValue hv = evaluate5Cards(combo);
...
int PokerEngine::getNextActivePlayer(int current) {
    for (int i = 1; i <= players.size(); ++i) {
        const int idx = (current + i) % players.size();
...
void PokerEngine::placeBet(int pIdx, int amount) {
    ...
    if (players[pIdx].currentBet > currentHighestBet) {
        const int raiseDiff = players[pIdx].currentBet - currentHighestBet;
...
void PokerEngine::handlePlayerDisconnect(int pIdx) {
    ...
    const QString disconnectedName = players[pIdx].name.isEmpty() ? QString(getLocalizedText("Игрок %1", "Player %1")).arg(pIdx + 1) : players[pIdx].name;
...
void PokerEngine::processAction(int pIdx, const QString& action, int raiseTotal) {
    ...
    if (action == "FOLD") {
        ...
        const int toCall = currentHighestBet - p.currentBet;
        placeBet(pIdx, std::min(toCall, p.balance));
    } else if (action == "RAISE") {
        AudioManager::instance().playSound(SoundEffect::ChipBet);
        const int amountToAdd = raiseTotal - p.currentBet;
...
void PokerEngine::resolveShowdown() {
    ...
    // Математический расчет основных и побочных банков (Side Pots)
    while (pot > 0) {
        ...
        for (int i = 0; i < players.size(); ++i) {
            if (players[i].totalContributed > 0) {
                const int take = std::min(players[i].totalContributed, minContr);
                segmentPot += take;
        ...
        const int split = segmentPot / std::max(1, static_cast<int>(segWinners.size()));
        const int rem = segmentPot % std::max(1, static_cast<int>(segWinners.size()));
    ...
    if (winnings.size() == 1) {
        const int wIdx = winnings.begin()->first;
...
void PokerEngine::updateStatus() {
    if (gameOver) return;
    const int toCall = currentHighestBet - players[currentTurnIdx].currentBet;
    if (toCall > 0) {
...
bool PokerEngine::makeAiMove() {
    ...
    const int toCall = currentHighestBet - bot.currentBet;
    const int randVal = QRandomGenerator::global()->bounded(100);
    ...
    if (communityCards.isEmpty()) {
        ...
        const int r1 = bot.holeCards[0].rank;
        const int r2 = bot.holeCards[1].rank;
        const bool isPair = (r1 == r2);
        const int highRank = std::max(r1, r2);
    ...
    const HandValue hv = evaluate7Cards(hand);
...
void PokerEngine::fromJson(const QJsonObject& json) {
    ...
    for (int i = 0; i < pArr.size(); ++i) {
        ...
        const int cardCount = pObj["cardCount"].toInt(0);
        players[i].holeCards.clear();
        const QJsonArray handArr = pObj["holeCards"].toArray();
